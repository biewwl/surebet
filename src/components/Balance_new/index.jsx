...
function Balance() {
  ...
  const [initial, normals, outflows, meta] = balance;
  ...
  const profitByOperation = results.map((r) => {
    return sumProfit(r);
  });

  const profit = profitByOperation.reduce((acc, val) => acc + val, 0);
  
  const current = profit + initial[0].value + normals[0].value - outflows[0].value;
  ...
  const metaClass = () => {
    if (current >= meta[0].value) {
      return " --done";
    } else if (current < meta[0].value) {
      return " --pending";
    }
  };

  const metaValue = () => {
    if (meta[0].value - current < 0) {
      return formatValue((meta[0].value - current) * -1);
    }
    return formatValue(meta[0].value - current);
  };
  ...
  const freebetCount = results.filter((r) => r[3].value).length;
        ...
        {balance.length > 0 && (
          <>
            <div className="balance__card content">
              ...
              {formatValue(profit + normals[0].value)}
            </div>
            <div className="balance__card content --meta">
              ...
              <p className="balance__card__detail">
                ...
                <span className="balance__card__detail__text">
                  * {formatValue(current + pending1Sum)}
                </span>
              </p>
              {/* {formatValue(current)} */}
            </div>
            ...
            <div className="balance__card content">
              ...
              <p className="balance__card__detail">
                {formatValue((profit / days).toFixed(2))}
